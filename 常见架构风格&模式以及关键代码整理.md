...
4. Unit Of Work
   ...
   - 逻辑视图:
		...
		graph LR
				 ...
				 printB --> stop
		```
   - 说明:类似三层架构、通过UnitOfWork对象、仓储以及EF完成增删改查和事务
...
2. EAP流程引擎DSL(数据流、虚拟机风格)
   ...
   - 逻辑视图:
		...
		graph LR
				 ...
				 printB --> stop
		```
   - 说明: 通过绘图画出EAP设备的交互流程->生成状态机描述代码->交由C#解释运行
...
3. 低代码引擎、设计器、组件库 DSL(虚拟机、黑板、独立构建风格)
   ...
   - 逻辑视图:
		...
		graph LR
				 ...
				 printB --> stop
		```
   - 说明:通过界面设计器设计出界面->生成界面的描述JSON(类似AST或者UI Tree)->交给渲染器渲染出界面并执行相关逻辑
...
4. 上、中、下位机通讯-生产者消费者模式(独立构建风格)
   ...
   - 逻辑视图:
		...
		graph LR
				 ...
				 printB --> stop
		```
   - 说明:下位机通过SECS协议同中位机(HOST)进行通讯和数据交互->再通过redis中的list数据结构配合生产者消费者模式,一读一写完成 中位机 同上位机的通讯->从而解耦下位机设备和上位机软件系统
...
7. 小型编译或解释器仅研究(数据流、虚拟机风格)
   ...
   - 逻辑视图:
		...
		graph LR
				 ...
				 printB --> stop
		```
   - 说明:通过Roslyn编译器开放出的接口读取符合一定规则的C#代码->逆向将C#代码转换为机器和EAP间进行交互的流程描述(JSON)->交由绘图程序绘制出机器和EAP交互的流程图
