...
4. Unit Of Work
   ...
   - 逻辑视图:
		...
		classDiagram
			...
			IRepository <|-- GenericRepository<T> : 实现
			GenericRepository<T> <|-- XXXRepository : 继承
		```
   - 说明:
...
7. 小型编译或解释器仅研究(数据流、虚拟机风格)
   ...
   - 逻辑视图:
		...
		```
   - 说明:
		通过Roslyn编译器开放出的接口读取符合一定规则的C#代码->逆向将C#代码转换为机器和EAP间进行交互的流程描述(JSON)->交由绘图程序绘制出机器和EAP交互的流程图
